...
export function Taskbar({ startOpen, onStart, onSearch, onLaunch }: { startOpen: boolean; onStart: () => void; onSearch: () => void; onLaunch: (label: string) => void }) {
  ...
  return <div className="taskbar-shell"><div className="taskbar-inner"><div className="hidden min-w-0 items-center gap-2 text-[10px] text-mist md:flex"><span className="status-dot"/> <span className="truncate">Joseph OS · Desktop ready</span></div><nav className="taskbar-apps" aria-label="Taskbar"><Button variant="taskbar" size="icon" onClick={onStart} aria-pressed={startOpen} aria-label="Open Joseph OS Start menu" className={startOpen ? "taskbar-active" : undefined}><JosephMark/></Button><Button variant="taskbar" size="icon" onClick={onSearch} aria-label="Search"><Search/></Button><span className="taskbar-divider"/>{shortcuts.map(({ label, icon: Icon }) => <Button key={label} variant="taskbar" size="icon" onClick={() => onLaunch(label)} aria-label={label} title={label}><Icon/></Button>)}</nav><SystemTray/></div></div>;
}
